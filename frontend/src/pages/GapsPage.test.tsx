...
describe("GapsPage", () => {
  ...
  it("filters gaps by core and extended catalog scope", async () => {
    ...
    expect(screen.getByText(/Exploit Public-Facing Application/i)).toBeInTheDocument();
    expect(screen.queryByText(/^DNS$/i)).not.toBeInTheDocument();
  });

  it("filters the gap matrix by selected gap categories", async () => {
    const user = userEvent.setup();
    ...
    await user.click(screen.getByRole("button", { name: "Detect only" }));

    expect(screen.getByText(/^DNS$/i)).toBeInTheDocument();
    expect(screen.queryByText(/Exploit Public-Facing Application/i)).not.toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: "reset" }));
    await user.click(screen.getByRole("button", { name: "Critical gaps" }));
